with open('frontend/src/features/cart/CartView.jsx', 'rb') as f:
    content = f.read()

# Fix the bad byte \xaa
# Let's see what is exactly around \xaa
# It is around: <div style={{ fontSize: '0.8rem', color: 'var(--text-light)', marginBottom: '0.25rem' }}>₪{pricePerMeter} למטר</div>

# Just decode with 'ignore' or 'replace', then we will replace the whole item block anyway!
...
import re
# Find where the target block starts approximately
idx = text.find('<div key={item.id} style={{ display: \'flex\', flexDirection: \'column\'')
if idx != -1:
    end_idx = text.find('                  </div>', idx)
    if end_idx != -1:
        end_idx += len('                  </div>')
        new_text = text[:idx] + replacement_str + text[end_idx:]
