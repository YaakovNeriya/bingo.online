...
const Login = () => {
  ...
  const [error, setError] = useState(isSessionExpired ? 'פג תוקף החיבור. אנא התחבר מחדש.' : '');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isLoginView) {
        if (email.toLowerCase().includes('facebook')) {
        ...
        if (!acceptedTerms) {
          setError('יש לאשר את תקנון האתר ומדיניות הפרטיות כדי להירשם');
          return;
        }
        if (email.toLowerCase().includes('facebook')) {
          localStorage.setItem('login_method', 'facebook');
          ...
          {!isLoginView && (
            <>
              <div>
                ...
                <input type="tel" className="input-field" value={phone} onChange={e => setPhone(e.target.value)} pattern="^[\d\s\-\+]+$" title="נא להזין מספר נייד תקין" required />
              </div>
              <div>
          ...
          {(!isSocialRegistration) && (
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                ...
                {isLoginView && (
                  <button type="button" onClick={() => navigate('/forgot-password', { state: { email } })} style={{ background: 'none', border: 'none', color: 'var(--primary-color)', cursor: 'pointer', fontSize: '0.9rem' }}>
                    שכחת סיסמה?
                  </button>
                )}
              </div>
          ...
          {!isLoginView && (
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: '0.5rem', marginTop: '0.25rem' }}>
              <input 
                ...
                required
              />
              <label htmlFor="terms" style={{ cursor: 'pointer', fontSize: '0.9rem', lineHeight: '1.4', userSelect: 'none' }}>
                אני מסכים/ה ל
                <a 
                  href="/terms" 
                  target="_blank" 
                  rel="noopener noreferrer" 
                  style={{ color: 'var(--primary-color)', textDecoration: 'underline', marginRight: '0.25rem', marginLeft: '0.25rem', fontWeight: '600' }}
