...
import { Trash2, Edit2, Upload, X, ArrowRight, ArrowLeft } from 'lucide-react';
  ...
  const handleModelSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        fabric_height: parseFloat(modelForm.fabric_height)
      };
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '1.5rem', marginTop: '1.5rem' }}>
        <div ref={typeFormRef} className="glass-panel" style={{ padding: '1.5rem' }}>
        ...
        <div ref={modelFormRef} className="glass-panel" style={{ padding: '1.5rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
          ...
          <form onSubmit={handleModelSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginTop: '1rem' }}>
            ...
            <input type="number" step="0.01" className="input-field" placeholder="גובה בד במטרים (ברירת מחדל: 1.5)" value={modelForm.fabric_height} onChange={e => setModelForm({ ...modelForm, fabric_height: e.target.value })} required />
            <button type="submit" className="btn btn-primary">{modelForm.id ? 'שמור שינויים' : 'צור דגם'}</button>
          </form>
        </div>

        <div ref={skuFormRef} className="glass-panel" style={{ padding: '1.5rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
          ...
          <form onSubmit={handleSkuSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginTop: '1rem' }}>
            ...
            <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
              ...
              <button type="button" className="btn btn-primary" onClick={() => fileInputRef.current.click()} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <Upload size={16} style={{ marginLeft: '0.5rem' }} /> העלה תמונות
              </button>
      ...
      <h3 style={{ marginTop: '3rem', marginBottom: '1rem' }}>קטלוג המוצרים הקיים</h3>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            {catalog.map(type => (
              <div key={type.id} style={{ borderBottom: '1px solid var(--glass-border)', paddingBottom: '1rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '0.5rem', background: 'rgba(59,130,246,0.25)', padding: '0.75rem 1rem', borderRadius: '8px', border: '1px solid rgba(59,130,246,0.4)' }}>
                  <h4 style={{ fontSize: '1.25rem', color: '#000000ff', margin: 0, fontWeight: 'bold' }}>{type.name}</h4>
                      ...
                      {model.color_skus.map(sku => (
                        <div key={sku.id} style={{ display: 'flex', alignItems: 'center', gap: '1rem', background: 'var(--glass-bg)', padding: '0.5rem 1rem', borderRadius: '6px', border: '1px solid var(--glass-border)', flex: '1 1 100%' }}>
                          {sku.image_urls && sku.image_urls.length > 0 ? (
                            ...
                            <span style={{ background: '#e2e8f0', color: '#64748b', padding: '2px 8px', borderRadius: '12px', fontSize: '0.8rem', fontWeight: 'bold', whiteSpace: 'nowrap' }}>
                              ללא תמונות
                            </span>
                          )}
